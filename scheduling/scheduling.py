...
def parse(s_input):
  return [Op(op[0], int(op[1]) , op[-2] if op[-2] != op[0] else '') for op in s_input.split()]
...
def conf(s):
  ...
  for i, op in enumerate(ops):
    ...
    confs += [
      (op, op2)
      for op2 in ops[i + 1:]
      if t != op2.transaction and o == op2.object and op2.action in conflicts
    ]
  return confs
...
def reads(s):
  ...
  for i, (x, t ,o) in enumerate(ops):
    if x == 'r':
      aborted = aborts(s)
      write_t = next((op.transaction for op in ops[i:] if op.action == 'w' and op.transaction not in aborted and op.transaction != t and op.object == o), None)
...
def aca(s):
  for ti, x, tj in reads(s):
    ...
    if ('c', tj, '') not in s[:index]:
      print('t{} wird nicht vor r{}({}) commited. NON-ACA.'.format(tj, ti, x))
      return False
    ...
    if x == 'w':
      ...
      for op in s[i:s.index(op_end)]:
        if op.transaction != t and op.object == o:
          print('{} < {} aber {} < {}. NON-ST'.format(writeop, op, op, op_end))
          return False
      print('auf {} wird vor {} von keinen anderen Transaktionen zugegriffen. OK.'.format(o, op_end))
  print('ST')
  return True
